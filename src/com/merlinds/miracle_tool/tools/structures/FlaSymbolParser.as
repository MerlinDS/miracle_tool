...
package com.merlinds.miracle_tool.tools.structures {
	...
	public class FlaSymbolParser {

		private var dataXML:XML;


		private var matrixFlags:Array = [
			"type",
			"ma", "mb", "mc", "md", "tx", "ty",
			"r", "g", "b", "tint", "alpha"
		];

		private var staticFlags:Array = [
			"type", "tx", "ty", "scaleX", "scaleY", "skewX", "skewY", "r", "g", "b", "tint", "alpha"
		];

		private var tweenFlags:Array = [
			"type", "index", "duration",
		...
		private var tweensArr:Array = [ curveParse([new Point(0, 0), new Point(0.3333, 0.3333), new Point(0.6667, 0.6667), new Point(1, 1)]) ];

		private var spritesArr:Array = [];
		private var transformArr_old:Array = [];
		private var framesArr:Array = [];
		private var labelsArr:Array = [];

		private var transformArr:Array = [];
		private var transformIndexesArr:Array = [];

		public function FlaSymbolParser() {
		}

		//==============================================================================
		//{region							PUBLIC METHODS

		public function execute(data:XML):Object {
			trace("Start parse symbol");
			var result:Object = {};
			var xmlnsPattern:RegExp = new RegExp("xmlns[^\"]*\"[^\"]*\"", "gi");
			this.dataXML = new XML(data.replace(xmlnsPattern, ""));
		...
		private function parseFrame(frames:XMLList){
		...
		private function pushTween(str:String):int{
			var index:int = -1
		...
		private function getTweenFromAcceleration(v:Number):String{
		...
		private function parseTweens(data:XMLList, flag:String):String{
...
//			saveFile(str);
		...
		private function saveFile(str:String){
		...
		private function addStatic(index:int, duration:int, data:XMLList){
		...
		private function getColorData(data:XMLList):Object{
		...
		private function getMatrixData(data:XMLList):Matrix{
			var symbolInstance:XMLList = data
		...
		private function shortestAngle(start:Number, end:Number):Number{
